...
function dcfSection(d, cur) {
  ...
  const methodNote = val.method === "ffo"
    ...
    ? (val.growth_stage1
      ? `Valued on a <span class='text-slate-300'>two-stage justified price-to-book</span> (residual-income) model: value = book + the present value of excess returns (ROE − r) × book, with book compounding at ${fmtPct(val.growth_stage1, 1)} for ${val.stage1_years || 10} years, then fading to the ${fmtPct(val.growth, 1)} terminal rate. A single-stage P/B, (ROE − g)/(r − g), can only use a terminal growth below your required return, so it under-prices a financial whose book compounds fast for years first. Uses through-cycle (7-yr median) ROE so a cyclical peak doesn't inflate it.`
  ...
  <section class="card rounded-2xl p-6">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
      ...
      <div class="bg-ink/40 rounded-lg p-2.5" title="Through-cycle return on equity, capped at 2× ROIC so leverage/goodwill can't inflate the multiple"><div class="text-[11px] text-muted">Return used (≤2× ROIC)</div><div class="text-sm font-semibold">${fmtPct(val.return_used, 1)}</div></div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
      ...
      <div class="bg-ink/40 rounded-lg p-2.5"><div class="text-[11px] text-muted">Required return</div><div class="text-sm font-semibold">${fmtPct(val.cost_of_equity, 1)}</div></div>
      ${val.growth_stage1 ? `<div class="bg-ink/40 rounded-lg p-2.5" title="Near-term book-value growth for stage 1 of the two-stage model, from the robust earnings trend (capped at 12%), then fading to the ${fmtPct(val.growth, 1)} terminal rate"><div class="text-[11px] text-muted">Near-term growth <span class="opacity-70">(→ ${fmtPct(val.growth, 1)})</span></div><div class="text-sm font-semibold text-brand">${fmtPct(val.growth_stage1, 1)}</div></div>` : ""}
    </div>
